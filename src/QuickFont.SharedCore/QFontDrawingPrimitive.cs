...
namespace QuickFont.SharedCore
{
    ...
    public class QFontDrawingPrimitive
    {
        ...
        private Vector2 TransformPositionToViewport(Vector2 input)
        {
            ...
            float x = (input.X - v2.Value.X)*(v1.GetValueOrDefault().Width/v2.Value.Width);
            float y = (input.Y - v2.Value.Y)*(v1.GetValueOrDefault().Height/v2.Value.Height);
        ...
        private SizeF TransformMeasureFromViewport(SizeF input)
        {
            ...
            Debug.Assert(v1 != null, "v1 != null");
            float x = input.Width*(v2.Value.Width/v1.GetValueOrDefault().Width);
            float y = input.Height*(v2.Value.Height/v1.GetValueOrDefault().Height);
        ...
        public static ProcessedText ProcessText(QFont font, QFontRenderOptions options, string text, SizeF maxSize, QFontAlignment alignment)
        {
            ...
            TextNodeList? nodeList = new TextNodeList(text);
