...
namespace QuickFont.SharedCore
{
    ...
    class QFontData
    {
        ...
        public List<string> Serialize()
        {
            ...
            foreach (KeyValuePair<char, QFontGlyph> glyphChar in CharSetMapping)
            {
                char chr = glyphChar.Key;
                QFontGlyph? glyph = glyphChar.Value;
        ...
        public void Deserialize(List<string> input, out int pageCount, out char[] charSet)
        {
            ...
            try
            {
                ...
                {
                    string[]? vals = input[2 + i].Split(' ');
                    QFontGlyph? glyph = new QFontGlyph(int.Parse(vals[1]), new Rectangle(int.Parse(vals[2]), int.Parse(vals[3]), int.Parse(vals[4]), int.Parse(vals[5])), int.Parse(vals[6]), vals[0][0]);
        ...
        {
            MeanGlyphWidth = 0f;
            foreach (KeyValuePair<char, QFontGlyph> glyph in CharSetMapping)
