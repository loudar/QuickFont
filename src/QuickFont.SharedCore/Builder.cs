...
using QuickFont.SharedCore.Configuration;

namespace QuickFont.SharedCore
{
    /// <summary>
    ...
    internal class Builder
    {
        ...
        {
            return glyphs.ToDictionary(t => t.Character);
        }
        ...
        private List<SizeF> GetGlyphSizes(IFont font)
        {
            ...
            List<SizeF> sizes = _charSet
                .Select(t => font.MeasureString("" + t, graph))
                .Select(charSize => new SizeF(charSize.Width + padding, charSize.Height + padding))
                .ToList();
        ...
        {
            SizeF min = GetMinGlyphSize(sizes);
            SizeF max = GetMaxGlyphSize(sizes);
        ...
        private Bitmap CreateInitialBitmap(IFont font, SizeF maxSize, int initialMargin, out QFontGlyph[] glyphs, TextGenerationRenderHint renderHint)
        {
            ...
            Bitmap bmp = new Bitmap(spacing * _charSet.Length, (int)Math.Ceiling(maxSize.Height) + 2 * initialMargin, PixelFormat.Rgb);
            Graphics graph = Graphics.FromImage(bmp);
            ...
            for (int i = 0; i < _charSet.Length; i++)
            {
				Point offset = font.DrawString("" + _charSet[i], graph, Brushes.White, xOffset, initialMargin);
                SizeF charSize = font.MeasureString("" + _charSet[i], graph);
        ...
        private static List<QBitmap> GenerateBitmapSheetsAndRepack(QFontGlyph[] sourceGlyphs, BitmapData[] sourceBitmaps, int destSheetWidth, int destSheetHeight, out QFontGlyph[] destGlyphs, int destMargin)
        {
            ...
            int maxY = 0;
            foreach (QFontGlyph glph in sourceGlyphs)
        ...
        public QFontData BuildFontData(string saveName = null)
        {
            ...
            List<SizeF> sizes = GetGlyphSizes(_font);
            SizeF maxSize = GetMaxGlyphSize(sizes);
            Bitmap initialBmp = CreateInitialBitmap(_font, maxSize, margin, out initialGlyphs,_config.TextGenerationRenderHint);

#if DEBUG 
			// print bitmap with bounds to debug it
			Bitmap? debugBmp = initialBmp.Clone() as Bitmap;
			Graphics graphics = Graphics.FromImage(debugBmp);
			Pen pen = new Pen(Color.Red, 1);

			foreach (QFontGlyph g in initialGlyphs)
			{
            ...
            char[]? intercept = FirstIntercept(fontData.CharSetMapping);
            if (intercept != null)
        ...
        private static QFont BuildDropShadow(List<QBitmap> sourceFontSheets, QFontGlyph[] sourceFontGlyphs, QFontShadowConfiguration shadowConfig, char[] charSet, byte alphaTolerance)
        {
            ...
            List<QBitmap> bitmapSheets = GenerateBitmapSheetsAndRepack(sourceFontGlyphs, sourceBitmapData.ToArray(), shadowConfig.PageMaxTextureSize, shadowConfig.PageMaxTextureSize, out newGlyphs, shadowConfig.GlyphMargin + shadowConfig.BlurRadius*3);
			...
			QFontData fontData = new QFontData {CharSetMapping = new Dictionary<char, QFontGlyph>()};
