...
namespace QuickFont.SharedCore
{
    ...
    public class QBitmap
    {
        ...
        public static void BlitMask(BitmapData source, BitmapData target, int srcPx, int srcPy, int srcW, int srcH, int px, int py)
        {
            ...
            int targetStartX = Math.Max(px, 0);
            int targetEndX = Math.Min(px + srcW, target.Width);

            int targetStartY = Math.Max(py, 0);
            int targetEndY = Math.Min(py + srcH, target.Height);
        ...
        public static void Blit(BitmapData source, BitmapData target, int srcPx, int srcPy, int srcW, int srcH, int destX, int destY)
        {
            ...
            int targetEndX = Math.Min(destX + srcW, target.Width);

            int targetStartY = Math.Max(destY, 0);
            int targetEndY = Math.Min(destY + srcH, target.Height);
